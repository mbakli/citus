...
ColumnarReadNextRow(TableReadState *readState, Datum *columnValues, bool *columnNulls)
{
	...
	/*
	 * We rely on first column's metadata in rest of this function. So for
	 * zero column tables we just return "true" for totalRowCount times.
	 */
	if (readState->tupleDescriptor->natts == 0)
	{
